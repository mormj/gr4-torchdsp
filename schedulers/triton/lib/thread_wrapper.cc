...
thread_wrapper::thread_wrapper(int id,
                               ...
                               const scheduler_triton_options& opts)
...
bool thread_wrapper::handle_work_notification()
{
    ...
    if (s == executor_iteration_status_t::DONE) {
        d_debug_logger->debug("Signalling DONE to RTMON");
        d_rtmon->push_message(
            rt_monitor_message::make(rt_monitor_message_t::DONE, id(), 0));
    }
    ...
    if (d_flushing) {
        ...
        bool all_blkd = true;
        if (s == executor_iteration_status_t::READY ||
            s == executor_iteration_status_t::READY_NO_OUTPUT ||
            s == executor_iteration_status_t::BLKD_OUT) {
            notify_self_ = true;
        } else if (s == executor_iteration_status_t::BLKD_IN) {
            // kick = true;
        }

        if (s == executor_iteration_status_t::MSG_ONLY) {
